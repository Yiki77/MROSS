...
parser.add_argument("--region_round", type=str,
                    help="Number of region_round")
...
target = f"{abs_path}/2-target_images/{args.target_file}"
...
def run(seed, wandb_name):
    exit_code = sp.run(["python", "painterly_rendering_single.py", target,
                        "--target_file", str(args.target_file),
                        "--region_round", str(args.region_round),
...
for seed in seeds:
    wandb_name = f"{test_name}_{args.num_strokes}_strokes_seed{seed}"
    if multiprocess:
